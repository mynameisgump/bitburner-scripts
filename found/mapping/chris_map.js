...
// Github Link: https://github.com/Chrisch3n/BitburnerScripts/blob/main/map.js
...
export async function main(ns) {
  ...
  if (
    ...
    ns.tprint(
      'Mapping: "Root access", [Backdoor], "Server name", ("Server Level", [Required Ports]), [Max. Money Server], [Hacking possible]'
    );
    if (controlSymbolTypeColor) {
      ns.tprint("- \u2705 = Root access / \u274C = No root access ");
      ns.tprint(
        "- [Optional] Backdoor: [BD] = Backdoor installed / [x] = No Backdoor"
      );
      ns.tprint(
        "- [Optional] Port Info: Variable behind Servername (Required Hacking Level - Required Server Ports)"
      );
      ns.tprint(
        '- If "!!!!!!" at the end is shown, your hacking and port hacking level is above the'
      );
      ns.tprint(
        "  requirements of the server but you dont have root access yet."
      );
    } else {
      ns.tprint("- ██ = Root access / [ ] = No root access ");
      ns.tprint(
        "- [Optional] Backdoor: [BD] = Backdoor installed / [NO BD] = No Backdoor"
      );
      ns.tprint(
        "- [Optional] Port Info: Variable behind Servername (Required Hacking Level - Required Server Ports)"
      );
      ns.tprint(
        '- "!!!!!!" at the end indicates, that your hacking and port hacking level is above the'
      );
      ns.tprint(
        "  requirements of the server but you dont have root access yet."
      );
    ...
    ns.tprint("");
    ns.tprint(
      "Note: Colored Symbols, Required Server Ports, Backdoor Indicator and Max. Money can be "
    );
    ns.tprint(
      "      (de)activated by changing the constants at the beginning of the script"
    );
    ns.tprint(
      "*********************************************************************************************************************"
    );
...
function PrintServerInfo(serverName, indent, prefix) {
  ...
  if (
    _ns.getHackingLevel() >= serverHackingLevel &&
    HackablePortsPlayer() >= serverPortLevel &&
    !serverinfo.hasAdminRights
  ) {
  ...
  if (controlMaxMoneyIndicator) {
    maxMoneyIndicator =
      " [Max Money" + _ns.nFormat(serverinfo.moneyMax, "($ 0.00a)") + "] ";
  }
  _ns.tprint(
    indentString +
      hacked +
